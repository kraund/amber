describe("Exercise 1", function () {
    it("should have the right title", async function () {
        ...
        await nxutils.asyncForEach(elements, async obj => {
        ...
        let secondAnswer = await getKeyByValue(result2, ourExpectedValues[1])
        await nxutils.waitUntilObjectIsVisibleAndClick(`[value='${secondAnswer}']`)

        let thirdAnswer = await getKeyByValue(result3, ourExpectedValues[2])
        await nxutils.waitUntilObjectIsVisibleAndClick(`[value='${thirdAnswer}']`)

        let fourthAnswer = await getKeyByValue(result4, ourExpectedValues[3])
        await nxutils.waitUntilObjectIsVisibleAndClick(`[value='${fourthAnswer}']`)

        await browser.pause(1000)

        const output = await browser.$$("<code />")
        const outputText = await output[5].getText()
